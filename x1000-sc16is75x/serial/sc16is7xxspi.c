...
static bool sc16is7xx_port_irq(struct sc16is7xx_port *s, int portno)
{
	...
	do {
		...
		if (iir & SC16IS7XX_IIR_NO_INT_BIT){
			//printk("sc16is7xx_port_irq disabled all interrupter!\n");
			return false;
		}
...
static void sc16is7xx_reconf_rs485(struct uart_port *port)
{
	...
	printk("sc16is7xx_reconf_rs485 start!\n");
	...
	printk("sc16is7xx_reconf_rs485 end!\n");
}
...
static void sc16is7xx_reg_proc(struct kthread_work *ws)
{
	...
	printk("sc16is7xx_reg_proc start!\n");
				  ...
				  struct serial_rs485 *rs485)
{
	struct sc16is7xx_port *s = dev_get_drvdata(port->dev);
	struct sc16is7xx_one *one = to_sc16is7xx_one(port, port);
	printk("sc16is7xx_config_rs485 start!\n");
	if (rs485->flags & SER_RS485_ENABLED) {
	...
	queue_kthread_work(&s->kworker, &one->reg_work);
	printk("sc16is7xx_config_rs485 end!\n");
	return 0;
}
			   ...
			   struct regmap *regmap, int irq, unsigned long flags)
{
	...
	// gpio_request(GPIO_PC(17), "sc16isq");
	// gpio_direction_input(GPIO_PC(17));
	...
	if (IS_ERR(s->clk)) {
		...
		freq = clk_get_rate(s->clk);//1843200;//
	}
printk("sc16is7xx_probe IS_ERR(s->clk) freq:%d,s->clk:%d\n",freq,s->clk);
	s->regmap = regmap;
	s->devtype = devtype;
	...
	for (i = 0; i < devtype->nr_uart; ++i) {
		...
		s->p[i].port.dev	= dev;
		s->p[i].port.irq	= irq;//gpio_to_irq(81);//
...
static int sc16is7xx_spi_probe(struct spi_device *spi)
{
	...
	struct regmap *regmap;
	int ret;
printk("start sc16is7xx_spi_probe!\n");
	/* Setup SPI bus */
	spi->bits_per_word	= 8;
