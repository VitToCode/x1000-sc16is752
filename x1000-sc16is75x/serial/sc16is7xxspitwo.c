...
#define SC16IS7XX_NAME1			"sc16is7xx1"
...
static struct uart_driver sc16is7xx_uart_1 = {//1130++
	.owner		= THIS_MODULE,
	.dev_name	= "ttySC",
	.nr		= SC16IS7XX_MAX_DEVS,
};

static struct serial_rs485 sc16is7xx_rs485 = {
...
static u8 sc16is7xx_port_read(struct uart_port *port, u8 reg)
{
	...
	// mutex_lock(&s->regmap->mutex);
	regmap_read(s->regmap, (reg << SC16IS7XX_REG_SHIFT) | line, &val);
	// mutex_unlock(&s->regmap->mutex);
...
static const struct sc16is7xx_devtype sc16is70_devtype = {
	.name		= "SC16IS7520",
	.nr_gpio	= 8,
	.nr_uart	= 2,
};
static const struct sc16is7xx_devtype sc16is71_devtype = {
	.name		= "SC16IS7521",
	.nr_gpio	= 8,
	.nr_uart	= 2,
};
			   ...
			   struct regmap *regmap, int irq, unsigned long flags)
{
	...
	struct sched_param sched_param1 = { .sched_priority = MAX_RT_PRIO / 2 +1};
	unsigned long freq, *pfreq = dev_get_platdata(dev);
	int i, ret;
	struct sc16is7xx_port *s;

	 printk("start sc16is7xx_probe!\n");

	struct spi_device *spidev = to_spi_device(dev);//1130++
	...
	if (IS_ERR(regmap))
	{
		printk("sc16is7xx_probe IS_ERR(regmap)!\n");
	...
	if (!s) {
		...
		printk("sc16is7xx_probe Error allocating port structure!\n");
		return -ENOMEM;
	}

	if (spidev->chip_select == 0)//1130++
	{
		//printk("[zoro] s->dev_id = 0\n");
		s->dev_id = 0;
	}
	else if(spidev->chip_select == 1)//
	{
		//printk("[zoro] s->dev_id = 1\n");
		s->dev_id = 1;
	}
	...
	if (IS_ERR(s->clk)) {
		...
		{
			//printk("sc16is7xx_probe IS_ERR(s->clk) freq:%d,s->clk:%d\n",freq,s->clk);
	...
	if(spidev->chip_select == 1)
		s->kworker_task = kthread_run(kthread_worker_fn, &s->kworker,
				      "sc16is7xx-1");
	else if (spidev->chip_select == 0)
		s->kworker_task = kthread_run(kthread_worker_fn, &s->kworker,
				      "sc16is7xx-0");

	if (IS_ERR(s->kworker_task)) {
		ret = PTR_ERR(s->kworker_task);
		printk("sc16is7xx_probe IS_ERR(s->kworker_task)\n");
		goto out_clk;
	}
	if (spidev->chip_select == 0){//1130++
		...
		sched_setscheduler(s->kworker_task, SCHED_FIFO, &sched_param1);
	}
	...
	if (devtype->nr_gpio) {
		...
		if (ret){
			printk("sc16is7xx_probe ret = gpiochip_add(&s->gpio)\n");
			goto out_thread;
		}
	}
#endif
	 printk("[zoro] nr_uart = %d ,%s \n", devtype->nr_uart, devtype->name);
	for (i = 0; i < devtype->nr_uart; ++i) {
		printk("[zoro] ceshi s->dev_id = %d\n", s->dev_id);
		s->p[i].line		= i;
		...
		if (s->p[i].port.line >= SC16IS7XX_MAX_DEVS) {
			ret = -ENOMEM;
			printk("sc16is7xx_probe s->p[i].port.line >= SC16IS7XX_MAX_DEVS\n");
			goto out_ports;
		}
		...
		if(spidev->chip_select == 1)
		{
			printk("[zoro] sc16is7xx_uart_1\n");
			uart_add_one_port(&sc16is7xx_uart_1, &s->p[i].port);
			s->dev_id = 1;
			printk("[zoro] ceshi2 s->dev_id = %d\n", s->dev_id);

		}
		else if(spidev->chip_select == 0)
		{
			printk("[zoro] sc16is7xx_uart\n");
			uart_add_one_port(&sc16is7xx_uart, &s->p[i].port);
			s->dev_id = 0;
			printk("[zoro] ceshi3 s->dev_id = %d\n", s->dev_id);
		}
		printk("[zoro] ceshi4 s->dev_id = %d\n", s->dev_id);

		/* Go to suspend mode */
		sc16is7xx_power(&s->p[i].port, 0);
		sc16is7xx_config_rs485(&s->p[i].port, &sc16is7xx_rs485);

		printk("[zoro] ceshi5 s->dev_id = %d, %x, %x \n", s->dev_id, client->addr, flags);
	}
	// printk("sc16is7xx_probe start devm_request_irq !\n");
	/* Setup interrupt */
	ret = devm_request_irq(dev, irq, sc16is7xx_irq,
			       flags, dev_name(dev), s);//| IRQF_SHARED
	
	if (!ret){
		printk("sc16is7xx_probe end devm_request_irq !\n");
...
out_ports:
	for (i--; i >= 0; i--) {
		...
		if(s->dev_id)
			uart_remove_one_port(&sc16is7xx_uart_1, &s->p[i].port);
		else
			uart_remove_one_port(&sc16is7xx_uart, &s->p[i].port);
...
static const struct of_device_id __maybe_unused sc16is7xx_dt_ids[] = {
	...
	{ .compatible = "nxp,sc16is7520",	.data = &sc16is70_devtype, },
	{ .compatible = "nxp,sc16is7521",	.data = &sc16is71_devtype, },
	{ }
};
...
static const struct spi_device_id sc16is7xx_spi_id_table[] = {
	...
	{ "sc16is7520",	(kernel_ulong_t)&sc16is70_devtype, },
	{ "sc16is7521",	(kernel_ulong_t)&sc16is71_devtype, },
	{ }
};
			       ...
			       const struct i2c_device_id *id)
{
	...
	printk("start sc16is7xx_i2c_probe!\n");
	...
	//printk("ret = sc16is7xx_i2c_probe!\n");
	//regmap->mutex = mutex;

	// mutex_init(&regmap->mutex);
	
	ret = sc16is7xx_probe(&i2c->dev, devtype, regmap, i2c->irq, flags);
	printk("end sc16is7xx_i2c_probe!\n");
	return ret;
}
...
{
	int ret;
	//printk("start uart_register_driver(&sc16is7xx_uart)\n");
	ret = uart_register_driver(&sc16is7xx_uart);
	if (ret) {
		pr_err("Registering UART driver failed\n");
		return ret;
	}
	ret = uart_register_driver(&sc16is7xx_uart_1);
	if (ret) {
		pr_err("Registering UART driver failed\n");
		return ret;
	}
	//printk("end uart_register_driver(&sc16is7xx_uart)\n");

#ifdef CONFIG_SERIAL_SC16IS7XX_I2C
	//printk("start i2c_add_driver(&sc16is7xx_i2c_uart_driver)!\n");
	ret = i2c_add_driver(&sc16is7xx_i2c_uart_driver);
	if (ret < 0) {
		pr_err("failed to init sc16is7xx i2c --> %d\n", ret);
		return ret;
	}
//	printk("end i2c_add_driver(&sc16is7xx_i2c_uart_driver)!\n");
#endif
...
#endif
	uart_unregister_driver(&sc16is7xx_uart_1);
